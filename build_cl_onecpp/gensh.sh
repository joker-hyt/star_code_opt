...
do
	...
	rm -rf $PROJ_FILE && mkdir $PROJ_FILE && cd $PROJ_FILE && cp ../genm.sh.template ./genm.sh && cp ../genonecpp.sh.template ./genonecpp.sh
	...
	sed -i "s/{{PROJ_FILE}}/$PROJ_FILE/g" ./genonecpp.sh
	sed -i "s/{{PROJ_PATH}}/$PROJ_PATH_LINUX/g" ./genonecpp.sh
	sed -i "s/{{PROJ_PCH_CPP}}/$PROJ_PCH_CPP/g" ./genonecpp.sh
	cd ..
done < config.in
